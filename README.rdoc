...
If you know english and another language, please help us translate by skipping to step 5 below (provide translations). It requires no special tools except the ability to 
...
* Text to be translated should be enclosed in _("text to translate") in *.py files.  In html templates translated text is indicated by:
...
* Obtain pybabel to perform the steps below.  You will need to install and compile {babel}[http://babel.edgewall.org/wiki/Download].
...
* Babel then needs to find all translationed text blocks throughout code and templates.
...
- Update translations of existing languages by running this command for each locale:
...
* Provide translations for each language
   ...
   <tt>msgid "Change your password"
   msgstr "Cambiar tu contraseña"</tt>
